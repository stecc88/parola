...
import { CORSI } from '@/lib/student/corso'

const LIVELLI = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'] as const

export function RegistrationForm({ simplifiedMode }: { simplifiedMode: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await registerStudent(
        nome.trim(),
        cognome.trim(),
        simplifiedMode ? null : livello,
        simplifiedMode ? corso : null,
        inviteCode.trim()
      )
        ...
        <Card className="relative w-full max-w-sm bg-surface shadow-xl text-center">
          ...
          {simplifiedMode ? (
            <div className="mb-4 rounded-md bg-info-bg px-3 py-3 text-sm text-info-text text-left">
              <p className="font-semibold mb-1">Il tuo account è già attivo</p>
              <p>Puoi accedere subito con il codice qui sopra, senza attendere approvazioni.</p>
            </div>
          ) : (
            <div className="mb-4 rounded-md bg-info-bg px-3 py-3 text-sm text-info-text text-left">
    ...
    >
      ...
      <Card className="relative w-full max-w-sm bg-surface shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {simplifiedMode ? (
            <div>
              <label htmlFor="corso" className="mb-1 block text-sm text-ink-secondary">Corso</label>
              <select
                id="corso"
                required
                value={corso}
