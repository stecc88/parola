...
function WritePageInner({ livelloIniziale }: { livelloIniziale: LivelloCefr }) {
  const searchParams = useSearchParams()
  const guida = getGuidaBySlug(searchParams.get('guida'))
  const [livello, setLivello] = useState<LivelloCefr>(livelloIniziale)
    ...
    <main id="main-content" className="mx-auto max-w-3xl p-6 animate-fade-in">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <LivelloSelector livelloIniziale={livelloIniziale} onLivelloChange={setLivello} />
        </div>
...
export function WritePageClient({ livelloIniziale }: { livelloIniziale: LivelloCefr }) {
  return (
    <Suspense>
      <WritePageInner livelloIniziale={livelloIniziale} />
    </Suspense>
  )
}
