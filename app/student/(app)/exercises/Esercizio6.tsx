...
export function Esercizio6() {
  const [stato, setStato] = useState<Stato>('idle')
  const [domande, setDomande] = useState<SituazioniComunicative['domande']>([])
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const result = await startEsercizio6()
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setValutazione(await submitEsercizio6(domande, arr))
      setStato('pronto')
    } catch (e) {
    ...
    return (
      <Card className="text-center">
        <p className="mb-4 text-sm text-ink-secondary">
          Scegli in quale situazione potresti usare ogni espressione.
        </p>
