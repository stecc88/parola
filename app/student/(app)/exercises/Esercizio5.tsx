...
import { startEsercizio5, submitEsercizio5 } from './actions'
import type { CompletamentoLessicale, ValutazioneRisposteStruttura } from '@/lib/gemini/prompts/struttura'
...
export function Esercizio5() {
  const [stato, setStato] = useState<Stato>('idle')
  const [testoIntroduttivo, setTestoIntroduttivo] = useState('')
  const [domande, setDomande] = useState<CompletamentoLessicale['domande']>([])
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const result = await startEsercizio5()
      setTestoIntroduttivo(result.testo_introduttivo)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setValutazione(await submitEsercizio5(domande, arr))
      setStato('pronto')
    } catch (e) {
    ...
    return (
      <Card className="text-center">
        <p className="mb-4 text-sm text-ink-secondary">
          Completa il testo scegliendo la parola giusta tra 4 opzioni.
        </p>
  ...
  if (stato === 'pronto' && valutazione) {
    ...
    return (
      <Card>
        ...
        <RisultatoFooter corretti={corretti} totale={valutazione.risultati.length} tipo={5} />
  ...
  return (
    <Card>
      {testoIntroduttivo && (
        <p className="mb-4 rounded-md bg-surface-secondary p-3 text-sm text-ink-secondary">
          {testoIntroduttivo}
        </p>
      )}
      ...
      {errore && (
        <p role="alert" className="mt-4 rounded-md bg-danger-bg px-3 py-2 text-sm text-danger-text">{errore}</p>
      )}
      <div className="mt-4 flex justify-end">
