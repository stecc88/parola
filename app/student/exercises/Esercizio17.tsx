...
import { startEsercizio17, submitEsercizio17 } from './actions'
import type { ClozeTestoC1, ValutazioneClozeC1 } from '@/lib/gemini/prompts/struttura'
import { RisultatoFooter } from './RisultatoFooter'

type Stato = 'idle' | 'generando' | 'rispondendo' | 'valutando' | 'pronto' | 'errore'

export function Esercizio17() {
  const [stato, setStato] = useState<Stato>('idle')
  const [esercizio, setEsercizio] = useState<ClozeTestoC1 | null>(null)
  const [risposte, setRisposte] = useState<Record<number, string>>({})
  const [risultato, setRisultato] = useState<ValutazioneClozeC1 | null>(null)
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const data = await startEsercizio17()
      setEsercizio(data)
      setRisposte({})
      setRisultato(null)
  ...
  async function handleSubmit() {
    ...
    try {
      const val = await submitEsercizio17(esercizio, arr)
      setRisultato(val)
      setStato('pronto')
    } catch (e) {
  ...
  if (stato === 'idle') {
    return (
      <Card className="text-center">
        <p className="mb-2 text-sm font-medium text-ink-primary">Completamento testuale — Formato C1</p>
        <p className="mb-4 text-sm text-ink-secondary">
          Brano argomentativo con lacune che richiedono connettivi, pronomi relativi complessi, costruzioni gerundiali, nominalizzazioni e strutture sintattiche avanzate C1.
        </p>
  ...
  if (stato === 'pronto' && risultato && esercizio) {
    ...
    return (
      <Card>
            ...
            <div key={r.numero} className={`rounded-md p-3 text-sm ${r.corretto ? 'bg-success-bg' : 'bg-danger-bg'}`}>
              <div className="flex flex-wrap items-start justify-between gap-1">
                <p className={`font-medium ${r.corretto ? 'text-success-text' : 'text-danger-text'}`}>
                  [{r.numero}] {r.corretto ? `✓ "${r.risposta_corretta}"` : `✗ Hai scritto: "${r.risposta_studente}" → "${r.risposta_corretta}"`}
                </p>
        ...
        <RisultatoFooter corretti={corretti} totale={totale} tipo={17} />
  ...
  return (
    <Card>
      ...
      <p className="mb-3 text-xs font-medium text-ink-secondary uppercase tracking-wide">
        Inserisci l'elemento corretto per ogni lacuna
      </p>
