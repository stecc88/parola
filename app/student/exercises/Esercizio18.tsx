...
import { startEsercizio18, submitEsercizio18 } from './actions'
...
export function Esercizio18() {
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const data = await startEsercizio18()
      setEsercizio(data)
      setRisposte({})
      setRisultato(null)
  ...
  async function handleSubmit() {
    ...
    try {
      const val = await submitEsercizio18(esercizio, arr)
      setRisultato(val)
      setStato('pronto')
    } catch (e) {
  ...
  if (stato === 'idle') {
    return (
      <Card className="text-center">
        <p className="mb-2 text-sm font-medium text-ink-primary">Scelta multipla morfolessicale — Formato C1</p>
        <p className="mb-4 text-sm text-ink-secondary">
          Brano saggistico con 15 lacune: scegli tra 4 opzioni testate su registro formale, collocazioni lessicali, connettivi con sfumatura diversa e morfologia avanzata C1.
        </p>
  ...
  if (stato === 'pronto' && risultato && esercizio) {
    ...
    return (
      <Card>
        ...
        <RisultatoFooter corretti={corretti} totale={totale} tipo={18} />
  ...
  return (
    <Card>
      ...
      <p className="mb-3 text-xs font-medium text-ink-secondary uppercase tracking-wide">
        Scegli la parola o struttura corretta per ogni lacuna
      </p>
