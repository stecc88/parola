...
import { startEsercizio12, submitEsercizio12 } from './actions'
import type { ClozePronomiB2, ValutazioneClozePronomi } from '@/lib/gemini/prompts/struttura'
import { RisultatoFooter } from './RisultatoFooter'

type Stato = 'idle' | 'generando' | 'rispondendo' | 'valutando' | 'pronto' | 'errore'

export function Esercizio12() {
  const [stato, setStato] = useState<Stato>('idle')
  const [esercizio, setEsercizio] = useState<ClozePronomiB2 | null>(null)
  const [risposte, setRisposte] = useState<Record<number, string>>({})
  const [risultato, setRisultato] = useState<ValutazioneClozePronomi | null>(null)
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const data = await startEsercizio12()
      setEsercizio(data)
      setRisposte({})
      setRisultato(null)
  ...
  async function handleSubmit() {
    ...
    try {
      const val = await submitEsercizio12(esercizio, arr)
      setRisultato(val)
      setStato('pronto')
    } catch (e) {
  ...
  if (stato === 'pronto' && risultato && esercizio) {
    ...
    return (
      <Card>
        ...
        <RisultatoFooter corretti={corretti} totale={totale} tipo={12} />
  ...
  return (
    <Card>
      ...
      <p className="mb-3 text-xs font-medium text-ink-secondary uppercase tracking-wide">
        Inserisci il pronome o aggettivo corretto
      </p>
        ...
        {esercizio.lacune.map((l) => (
          <div key={l.numero} className="flex items-center gap-3">
            <div className="shrink-0 text-right">
              ...
              <p className="mt-0.5 text-xs italic text-brand-500">{l.tipo}</p>
            </div>
            <input
