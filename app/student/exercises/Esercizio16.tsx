...
import { startEsercizio16, submitEsercizio16 } from './actions'
...
export function Esercizio16() {
  ...
  async function handleStart() {
    ...
    setStato('generando')
    try {
      const data = await startEsercizio16()
      setEsercizio(data)
      setRisposte({})
      setRisultato(null)
  ...
  async function handleSubmit() {
    ...
    try {
      const val = await submitEsercizio16(esercizio, arr)
      setRisultato(val)
      setStato('pronto')
    } catch (e) {
  ...
  if (stato === 'idle') {
    return (
      <Card className="text-center">
        <p className="mb-2 text-sm font-medium text-ink-primary">Cloze verbi su testo — Formato C1</p>
        <p className="mb-4 text-sm text-ink-secondary">
          Brano con lacune verbali C1: coniuga ogni verbo (tra parentesi) includendo gerundio, congiuntivo passato/trapassato, participio assoluto e passiva avanzata.
        </p>
  ...
  if (stato === 'pronto' && risultato && esercizio) {
    ...
    return (
      <Card>
        ...
        <RisultatoFooter corretti={corretti} totale={totale} tipo={16} />
